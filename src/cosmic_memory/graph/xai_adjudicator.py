...
class XAIEntityAdjudicationService:
    ...
    def _adjudicate_once(
        self,
        request: EntityAdjudicationRequest,
    ) -> tuple[EntityAdjudicationDecision, object]:
        ...
        if hasattr(chat, "parse"):
            raw_response = chat.parse(EntityAdjudicationDecision)
        else:
            sampled = chat.sample()
            raw_response = getattr(sampled, "content", sampled)
        return _coerce_adjudication_result(raw_response), raw_response
